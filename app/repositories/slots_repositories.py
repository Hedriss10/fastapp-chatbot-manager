...
class SlotsRepositories:
    ...
    async def list_slots(
        ...
        Args:
            ...
            slot_minutes (int, optional): Duração do slot em minutos. Default: 30.
            target_date (date, optional): Data para gerar os slots. Default: hoje.

        Returns:
            List[Dict[str, datetime]]: Lista de slots disponíveis com start/end.
        """
        try:
            target_date = data.target_date or datetime.now().date()

            # 1. Intervalo de trabalho
            start_dt = datetime.combine(target_date, datetime.strptime(data.work_start, '%H:%M').time())
            end_dt = datetime.combine(target_date, datetime.strptime(data.work_end, '%H:%M').time())

            # 2. Gera slots brutos
            slots = []
            current = start_dt
            while current < end_dt:
                slots.append({'start': current, 'end': current + timedelta(minutes=data.slot_minutes)})
            ...
            block_results = await self.session.execute(
                select(self.block.start_time, self.block.end_time).where(self.block.employee_id == data.employee_id)
            )
